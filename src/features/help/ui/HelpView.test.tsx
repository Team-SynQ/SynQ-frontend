...
describe('HelpView', () => {
  it('renders the readable SynQ introduction state', () => {
    ...
    const scrollColumn = boardFrame?.parentElement
    ...
    expect(scrollColumn).toHaveClass('flex-1', 'overflow-y-auto', 'pt-[44px]', 'w-[880px]')
    expect(scrollColumn?.className).not.toContain('ml-[168px]')
    expect(scrollColumn?.className).not.toContain('self-start')
    // 이미지·점·텍스트·버튼이 한 컬럼에 있어 화면 높이와 무관하게 간격이 고정됩니다.
    expect(scrollColumn).toContainElement(introductionTitle.parentElement)
    expect(scrollColumn).toContainElement(screen.getByRole('status'))
    expect(scrollColumn).toContainElement(screen.getByRole('button', { name: '다음' }))
  })
  ...
  it('switches tabs and advances the meeting tutorial frame', async () => {
    ...
    expect(meetingFrame.parentElement).toHaveClass('w-[880px]')
    expect(
