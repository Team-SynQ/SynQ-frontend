...
describe('ProjectMaterialUploadForm', () => {
  ...
  it('limits a file batch to five items before calling the upload adapter', () => {
    ...
    const files = Array.from({ length: 6 }, (_, index) => (
      new File(['content'], `brief-${index}.pdf`, { type: 'application/pdf' })
    ))
    fireEvent.change(
  ...
  it('maps upload adapter errors to the matching toast', async () => {
    ...
    const onUploadFiles = vi.fn().mockRejectedValue(
      new ProjectMaterialUploadError('too-many-files'),
    )
    ...
    expect(
      await screen.findByRole('status', { name: '동시 업로드 개수 초과' }),
    ).toBeInTheDocument()
  })
  ...
  it('falls back to the general failure toast for unknown upload errors', async () => {
    ...
    expect(
      await screen.findByRole('status', { name: '파일 업로드 실패' }),
    ).toBeInTheDocument()
    expect(
      screen.getByText('파일을 업로드하지 못했습니다. 다시 시도해 주세요'),
    ).toBeInTheDocument()
  })
  ...
  it('shows the link failure toast for unsupported links', async () => {
    ...
    await user.type(
      screen.getByPlaceholderText('링크를 입력하세요'),
  ...
  it('shows upload progress and completion states from the upload promise', async () => {
    ...
    const onUploadFiles = vi.fn(() => new Promise<void>((resolve) => {
    ...
    expect(
      await screen.findByRole('img', { name: '업로드 완료' }),
    ).toBeInTheDocument()
  ...
  it('keeps the modal open and shows a toast when project creation fails', async () => {
    ...
    expect(
      await screen.findByRole('status', { name: '프로젝트 생성 실패' }),
    ).toHaveTextContent('프로젝트를 생성하지 못했습니다. 다시 시도해 주세요.')
    expect(
